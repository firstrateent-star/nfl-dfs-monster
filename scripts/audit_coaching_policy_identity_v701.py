...
def main() -> None:
    ...
    renames = (
        (
            ...
            "v701_coaching_policy_identity_matrix.csv",
        ),
        (
            "v635_coaching_policy_identity_summary.csv",
            "v701_coaching_policy_identity_summary.csv",
        ),
    )
    ...
    new_report = out / "v701_coaching_policy_identity.json"
    payload = json.loads(old_report.read_text(encoding="utf-8"))
    payload["experiment"] = "v7.0.1-qb-rush-family-coaching-policy-identity"
    payload["audit_logic"] = (
        "frozen coaching audit with authoritative v7.1 shadow runtime"
    )
