...
def summarize_game(result: GameResultV13) -> GameAnatomySummary:
    ...
    return GameAnatomySummary(
        snaps=len(p), scrimmage_plays=pass_plays + run_plays,
        pass_plays=pass_plays, pass_attempts=pass_attempts, dropbacks=pass_plays,
        run_plays=run_plays, rush_attempts=run_plays + scrambles,
        scrambles=scrambles, sacks=sacks,
        ...
        field_goals_made=sum(1 for x in result.special_teams_events if x.event_type == "field_goal" and x.made),
        touchdowns=sum(x.touchdown for x in p), pressures=sum(x.pressured for x in p),
        stuffs=sum(x.stuffed for x in p), drives=result.drives,
        away_points=result.final_state.away_score, home_points=result.final_state.home_score,
        try_attempts=len(result.try_events), try_points=sum(x.points for x in result.try_events),
...
def assert_event_conservation(result: GameResultV13) -> None:
    ...
    if s.snaps != s.scrimmage_plays+s.punts+s.field_goal_attempts: raise AssertionError("recorded events must partition into scrimmage, punts, and field goals")
    if s.dropbacks != s.pass_attempts+s.sacks+s.scrambles: raise AssertionError("dropbacks must partition into throws, sacks, and scrambles")
    if pass_attempts != s.pass_attempts: raise AssertionError("player pass attempts must equal event-derived throws")
    ...
    if rush_attempts != s.rush_attempts: raise AssertionError("rush attempts must equal designed runs plus QB scrambles")
