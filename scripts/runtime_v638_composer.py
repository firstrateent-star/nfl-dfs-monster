...
def runtime_fingerprint_v638() -> RuntimeFingerprintV638:
    payload = _fingerprint_payload()
    digest = hashlib.sha256(
        json.dumps(payload, sort_keys=True, separators=(",", ":")).encode("utf-8")
    ).hexdigest()
    return RuntimeFingerprintV638(**payload, runtime_hash=digest)
...
def _runtime_integrity_errors(*, require_role_guard: bool) -> list[str]:
    ...
    if require_role_guard:
        if reality_v62.sample_target_share_plan is not sample_target_share_plan_v635:
            errors.append("v6.3.5 target role sampler changed inside v6.3.8")
        if reality_v62.sample_event_rush_share_plan is not sample_rush_share_plan_v638:
            errors.append("v6.3.8 rush role sampler is not active after personnel compile")
    return errors


def compose_v638_runtime() -> None:
    """Install frozen v6.3.6 identity plus the isolated v6.3.8 gadget decomposition."""

    v634.install_current_role_guard = install_current_role_guard_v638
    ...
    if errors:
        raise RuntimeError(
            "v6.3.8 static runtime integrity failure: " + "; ".join(errors)
        )


def assert_v638_runtime() -> RuntimeFingerprintV638:
    errors = _runtime_integrity_errors(require_role_guard=True)
    if errors:
        raise RuntimeError("v6.3.8 runtime integrity failure: " + "; ".join(errors))
    return runtime_fingerprint_v638()


def assert_team_runtime_v638(teams: dict[str, object]) -> None:
    missing_flow = [
        team_id
...
def build_week1_runtime_inputs_v638(
    *,
    ...
    assert_team_runtime_v638(teams)
    fingerprint = assert_v638_runtime()
...
def write_runtime_fingerprint_v638(out: Path) -> RuntimeFingerprintV638:
    fingerprint = assert_v638_runtime()
    out.mkdir(parents=True, exist_ok=True)
    (out / "runtime_fingerprint_v638.json").write_text(
