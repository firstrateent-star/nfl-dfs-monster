...
def simulate_slate(games, worlds: int, base_seed: int, out_dir: Path) -> list[dict]:
    ...
    for i, game in enumerate(games):
        ...
        np.savez_compressed(
            ...
            away_drives=result.away_drives,
            home_drives=result.home_drives,
            away_touchdowns=result.away_touchdowns,
            home_touchdowns=result.home_touchdowns,
            away_field_goals=result.away_field_goals,
            home_field_goals=result.home_field_goals,
        ...
        summaries.append(
            {
                ...
                "p50": float(np.quantile(total, 0.50)),
                ...
                "away_td_mean": float(result.away_touchdowns.mean()),
                "home_td_mean": float(result.home_touchdowns.mean()),
                "artifact": str(path),
