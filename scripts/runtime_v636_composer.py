...
def runtime_fingerprint_v636() -> RuntimeFingerprintV636:
    payload = _fingerprint_payload()
    digest = hashlib.sha256(
        json.dumps(payload, sort_keys=True, separators=(",", ":")).encode("utf-8")
    ).hexdigest()
    return RuntimeFingerprintV636(**payload, runtime_hash=digest)
...
def _runtime_integrity_errors(*, require_role_guard: bool) -> list[str]:
    ...
    if require_role_guard:
        if reality_v62.sample_target_share_plan is not sample_target_share_plan_v635:
            errors.append("v6.3.5 target role sampler changed inside v6.3.6")
        if reality_v62.sample_event_rush_share_plan is not sample_rush_share_plan_v636:
            errors.append("v6.3.6 rush role sampler is not active after personnel compile")
    return errors


def compose_v636_runtime() -> None:
    """Install v6.3.5 production reality plus the isolated v6.3.6 rush fix."""

    v634.install_current_role_guard = install_current_role_guard_v636
    ...
    if errors:
        raise RuntimeError(
            "v6.3.6 static runtime integrity failure: " + "; ".join(errors)
        )


def assert_v636_runtime() -> RuntimeFingerprintV636:
    errors = _runtime_integrity_errors(require_role_guard=True)
    if errors:
        raise RuntimeError("v6.3.6 runtime integrity failure: " + "; ".join(errors))
    return runtime_fingerprint_v636()


def assert_team_runtime_v636(teams: dict[str, object]) -> None:
    missing_flow = [
        team_id
...
def build_week1_runtime_inputs_v636(
    *,
    ...
    assert_team_runtime_v636(teams)
    fingerprint = assert_v636_runtime()
...
def write_runtime_fingerprint_v636(out: Path) -> RuntimeFingerprintV636:
    fingerprint = assert_v636_runtime()
    out.mkdir(parents=True, exist_ok=True)
    (out / "runtime_fingerprint_v636.json").write_text(
