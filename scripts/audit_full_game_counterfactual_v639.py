...
def main() -> None:
    ...
    # authoritative v6.3.9 runtime so the diagnostic and production simulator execute
    # the same machine.
    ...
    new_worlds = out / "v639_full_game_qb_counterfactual_worlds.csv"
    if old_worlds.exists():
        old_worlds.replace(new_worlds)

    old_report = out / "v635_full_game_qb_counterfactual.json"
    new_report = out / "v639_full_game_qb_counterfactual.json"
    payload = json.loads(old_report.read_text(encoding="utf-8"))
    payload["experiment"] = "v6.3.9-hierarchical-full-game-qb-counterfactual"
    payload["audit_logic"] = "exact v6.3.5 hierarchical audit, v6.3.9 runtime injected"
