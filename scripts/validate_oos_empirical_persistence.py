...
CONVERSION = {
    ...
    "defensive_td_drive_rate_allowed", "defensive_fg_drive_rate_allowed", "defensive_takeaway_drive_rate",
}
EFFICIENCY = {
    "offensive_epa_per_play", "offensive_success_rate", "offensive_explosive_rate",
    "defensive_epa_allowed_per_play", "defensive_explosive_rate_allowed", "defensive_sack_rate", "defensive_qb_hit_rate",
}

VARIANTS = ("latest", "empirical_all", "empirical_efficiency", "empirical_nonbehavior")


def _team_dev_map(frame: pl.DataFrame, feature: str) -> dict[str, float]:
    mean = _mean(frame, feature)
    out: dict[str, float] = {}
    for row in frame.select(["team_id", feature]).to_dicts():
        value = row.get(feature)
        out[str(row["team_id"])] = (mean if value is None else float(value)) - mean
    return out


def _persistence(older: pl.DataFrame, latest: pl.DataFrame, feature: str) -> float:
    a = _team_dev_map(older, feature)
    b = _team_dev_map(latest, feature)
    teams = [t for t in NFL_TEAMS if t in a and t in b]
    if len(teams) < 10:
        return 0.50
    x = np.array([a[t] for t in teams], dtype=float)
    y = np.array([b[t] for t in teams], dtype=float)
    if np.std(x) <= 1e-12 or np.std(y) <= 1e-12:
        return 0.35
    rho = float(np.corrcoef(x, y)[0, 1])
    # Negative year-to-year team identity is not promoted as a reversal rule here;
    # it means the feature has little stable team-level authority across an offseason.
    return float(np.clip(rho, 0.10, 0.90))


def _build_policy(older: pl.DataFrame, latest: pl.DataFrame, variant: str) -> tuple[pl.DataFrame, dict[str, float]]:
    if variant == "latest":
        return latest.select(["team_id", *[f for f in STATE_FEATURES if f in latest.columns]]), {f: 1.0 for f in STATE_FEATURES}

    persistence = {f: _persistence(older, latest, f) for f in STATE_FEATURES}
    old_rows = {str(r["team_id"]): r for r in older.to_dicts()}
    latest_rows = {str(r["team_id"]): r for r in latest.to_dicts()}
    output: list[dict[str, float | str]] = []

    if variant == "empirical_efficiency":
        adaptive = EFFICIENCY
    elif variant == "empirical_nonbehavior":
        adaptive = CONVERSION | EFFICIENCY
    else:
        adaptive = set(STATE_FEATURES)
    ...
    for team in NFL_TEAMS:
        ...
        for feature in STATE_FEATURES:
            ...
            lv = latest_rows.get(team, {}).get(feature)
            ov = old_rows.get(team, {}).get(feature)
            lv = lm if lv is None else float(lv)
            ov = om if ov is None else float(ov)
            if feature not in adaptive:
                row[feature] = lv
                continue

            rho = persistence[feature]
            latest_dev = lv - lm
            older_dev = ov - om
            # Older identity only earns weight when the feature demonstrates persistence.
            older_weight = 0.25 * rho
            blended_dev = (1.0 - older_weight) * latest_dev + older_weight * older_dev
            row[feature] = float(lm + rho * blended_dev)
        output.append(row)
    return pl.DataFrame(output).sort("team_id"), persistence


def run_fold(older_season: int, latest_season: int, test_season: int, worlds: int, seed: int) -> tuple[list[dict], pl.DataFrame, list[dict]]:
    ...
    for variant in VARIANTS:
        policy, persistence = _build_policy(older, latest, variant)
        policies[variant] = policy
        if variant == "empirical_all":
            for feature, rho in persistence.items():
                persistence_rows.append({
                    ...
                    "feature": feature,
                    "persistence": rho,
                    "group": "behavior" if feature in BEHAVIOR else ("conversion" if feature in CONVERSION else "efficiency"),
                })
    ...
    return metrics, frame, persistence_rows


def main() -> None:
    p = argparse.ArgumentParser()
    p.add_argument("--worlds", type=int, default=2500)
    p.add_argument("--seed", type=int, default=2026090751)
    p.add_argument("--out", type=Path, default=Path("artifacts/oos-empirical-persistence"))
    args = p.parse_args()
    ...
    for i, fold in enumerate(folds):
        m, g, r = run_fold(*fold, args.worlds, args.seed + i * 10_000_019)
        metrics.extend(m); games.append(g); persistence.extend(r)

    mf = pl.DataFrame(metrics)
    pf = pl.DataFrame(persistence)
    ...
    passed = bool(
        best["total_mae_gain"] > 0 and best["margin_mae_gain"] > 0
        and best["total_corr_gain"] >= 0 and best["margin_corr_gain"] >= 0
        and best["positive_both_folds"] >= 3
    )

    persistence_summary = pf.group_by("group").agg(
        pl.col("persistence").mean().alias("mean_persistence"),
        pl.col("persistence").median().alias("median_persistence"),
        pl.col("persistence").min().alias("min_persistence"),
        pl.col("persistence").max().alias("max_persistence"),
    ).sort("group")

    manifest = {
        "artifact": "Monster Empirical Feature Persistence OOS Ablation",
        ...
        "persistence_summary": persistence_summary.to_dicts(),
        "best_candidate": best,
        "robustness_pass": passed,
        "market_blind": True,
        "principle": "Offseason persistence is measured from information available before the forecast season; each mechanism earns its own shrinkage authority from prior year-to-year stability.",
    }
    args.out.mkdir(parents=True, exist_ok=True)
    pl.concat(games).write_csv(args.out / "games.csv")
    mf.write_csv(args.out / "fold_metrics.csv")
    pf.write_csv(args.out / "feature_persistence.csv")
    persistence_summary.write_csv(args.out / "persistence_summary.csv")
