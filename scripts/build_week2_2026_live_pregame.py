...
def freeze_inputs(inputs: dict[str, pl.DataFrame]) -> dict[str, pl.DataFrame]:
    ...
    # Critical leakage guard: Week 2 game snaps/results remain unavailable even
    # though live roster/depth/injury information is refreshed through Saturday.
    ...
    frozen["depth_charts"] = _date_filter(depth, "dt")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Build live 2026 Week 2 personnel using Saturday pre-lock "
            "roster/depth/injury knowledge while hard-blocking Week 2 snaps."
        )
    )
    ...
    if args.season != 2026:
        raise ValueError("Live Week 2 freezer is intentionally specific to 2026")
    ...
    manifest["benchmark_input_freeze"] = {
        "name": "week2_2026_live_pregame_2026-09-19",
        ...
        "live_week2_roster_depth_injury_news_allowed": True,
        ...
        "week2_game_results_available_to_builder": False,
        "market_blind": True,
    }
