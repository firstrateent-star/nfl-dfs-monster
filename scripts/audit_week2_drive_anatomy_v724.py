        ...
        rows.append(
            {
                ...
                "actual_week2": a,
                "historical_2025": h,
                "v724_minus_actual": s - a,
                "v724_vs_actual_relative": _relative_delta(s, a),
                "v724_minus_historical": s - h,
                "v724_vs_historical_relative": _relative_delta(s, h),
...
def _team_comparison(
    ...
    for key in sorted(actual_groups):
        ...
        sim = _summary(sim_groups[key], team_games=worlds, scope=f"v724:{game}:{team}")
        output.append(
            {
                "game": game,
                "team": team,
                "actual_drives": actual["drives"],
                "v724_drives_mean": sim["drives_per_team_game"],
                "actual_points_per_drive": actual["points_per_drive"],
                "v724_points_per_drive": sim["points_per_drive"],
                ...
                "v724_offensive_points_mean": sim["offensive_points_per_team_game"],
                ...
                "v724_td_rate": sim["touchdown_rate"],
                "actual_punt_rate": actual["punt_rate"],
                "v724_punt_rate": sim["punt_rate"],
                "actual_turnover_rate": actual["turnover_rate"],
                "v724_turnover_rate": sim["turnover_rate"],
                "actual_yards_per_drive": actual["net_yards_per_drive"],
                "v724_yards_per_drive": sim["net_yards_per_drive"],
                "actual_rz_snap_rate": actual["red_zone_snap_rate"],
                "v724_rz_snap_rate": sim["red_zone_snap_rate"],
                "actual_rz_td_conversion": actual["td_per_red_zone_snap"],
                "v724_rz_td_conversion": sim["td_per_red_zone_snap"],
            }
        )
    return output
...
def main() -> None:
    ...
    sim_summary = _summary(
        sim_rows, team_games=games * 2 * worlds, scope="v724_week2_simulation"
    )
    ...
    structural.sort(
        key=lambda row: abs(float(row["v724_vs_historical_relative"] or 0.0)),
        reverse=True,
    )
    week2_specific = [
    ...
    diagnosis = {
        ...
        "v724_vs_2025_points_per_drive_relative": _relative_delta(
            float(sim_summary["points_per_drive"]),
    ...
    segment_rows = _segment_rows(
        sim_rows, team_games=games * 2 * worlds, source="v724"
    ) + _segment_rows(
    ...
    manifest = {
        "artifact": "MONSTER V7.2.4 Week 2 Drive Anatomy Reality Audit",
        "season": args.season,
        "week": args.week,
        "games": games,
