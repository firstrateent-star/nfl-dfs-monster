...
def _date_filter(
    ...
    *,
    ...
    parsed = [
        pl.col(column).cast(pl.Datetime, strict=False).dt.date()
        for column in available
    ]
    has_any = parsed[0].is_not_null()
    before = parsed[0].is_null() | (parsed[0] <= pl.lit(CUTOFF_DATE))
    for expr in parsed[1:]:
        has_any = has_any | expr.is_not_null()
        before = before & (expr.is_null() | (expr <= pl.lit(CUTOFF_DATE)))
    return frame.filter((before & has_any) if fail_closed else before)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Build a single leakage-resistant 2026 Week 2 input freeze before "
            "the Thursday opener, allowing Week 1 evidence but no Week 2 results."
        )
    )
    ...
    if args.season != 2026:
        raise ValueError("strict Week 2 freezer is intentionally specific to 2026")
    ...
    manifest["benchmark_input_freeze"] = {
        "name": "week2_2026_strict_preopener_2026-09-16",
        "cutoff_date": CUTOFF_DATE.isoformat(),
        "first_week2_game_date": "2026-09-17",
