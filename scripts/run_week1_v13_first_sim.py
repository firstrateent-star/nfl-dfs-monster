...
def _defensive_unit(players) -> DefensiveUnit:
    ...
    for p in players:
        ...
        if pos in {"DE", "DT", "NT", "DL", "EDGE", "LB", "ILB", "OLB", "MLB"}:
            front.append(item)
        if pos in {"CB", "DB", "S", "FS", "SS", "LB", "ILB", "OLB", "MLB"}:
...
def _team_identity(team_id: str, pool, reality, unit_players, state) -> TeamIdentity:
    ...
    qb_state = max(qbs, key=lambda p: p.qb_pass_share)
...
def main() -> None:
    ...
    teams = {team: _team_identity(team, pools[team], reality, units[team], states[team]) for pair in MATCHUPS for team in pair}
    ...
    for game, metrics in anatomy_acc.items():
        ...
        for key, values in metrics.items():
            ...
            if key in {"away_points", "home_points", "snaps", "drives"}:
