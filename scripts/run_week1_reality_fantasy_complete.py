...
from monster.sim.pipeline_v12 import simulate_monster_game as simulate_monster_game_v12
...
def main() -> None:
    ...
    # Reuse certified Full-Reality evidence compilation while replacing only the causal kernel.
    # v1.2: possessions -> finite plays -> field state -> scoring events -> inferred scoreboard.
    # Player/fantasy scoring remains downstream of the football world.
    ...
    def full_pool_compile(*args, **kwargs):
        ...
        return {
            team_id: (apply_environment_to_pool(pool, inputs) if (inputs := environment_inputs(team_id)) is not None else pool)
    ...
    fantasy.simulate_monster_game = simulate_monster_game_v12
