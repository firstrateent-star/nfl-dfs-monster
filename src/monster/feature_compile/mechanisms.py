...
class PlayerMechanismInputs:
    ...
    madden_attributes: MaddenAttributeVector = MaddenAttributeVector()
...
def compile_player_mechanisms(base: PlayerState, inputs: PlayerMechanismInputs) -> tuple[PlayerState, PlayerMechanismTrace]:
    ...
    route_rating = inputs.madden_route_running or channel_mean(inputs.madden_attributes, "receiver_release")
    ...
    passing = _weighted_signal(((_madden_signal(inputs, "qb_accuracy"), 0.75), (_madden_signal(inputs, "qb_arm"), 0.25)))
    ball_security = _madden_signal(inputs, "ball_security")

    speed_signal = _weighted_signal(((forty_signal, 0.45), (madden_speed_signal, 0.25), (acceleration_signal, 0.15), (movement, 0.15)))
    catchpoint_signal = _weighted_signal(((height_signal, 0.20), (wingspan_signal, 0.20), (catching_signal, 0.35), (route_signal, 0.25)))
    rushing_signal = _weighted_signal(((forty_signal, 0.20), (madden_speed_signal, 0.15), (acceleration_signal, 0.15), (weight_signal, 0.20), (open_field, 0.30)))

    explosive_modifier = float(np.clip(base.explosive_modifier * (1.0 + 0.05 * speed_signal), 0.94, 1.06))
    catchpoint_modifier = float(np.clip(base.catchpoint_modifier * (1.0 + 0.05 * catchpoint_signal), 0.94, 1.06))
    rushing_modifier = float(np.clip(base.rushing_efficiency_modifier * (1.0 + 0.05 * rushing_signal), 0.94, 1.06))
    ...
    if inputs.age_years is not None and inputs.career_workload is not None:
        ...
        workload_signal = max(_signal(inputs.career_workload, ref["workload"], ref["workload"] * 0.55), 0.0)
        biology_uncertainty = float(np.clip(0.025 * age_excess * workload_signal, 0.0, 0.05))

    continuity_signal = 0.0
    if inputs.unit_continuity is not None:
        continuity_signal = float(np.clip((float(inputs.unit_continuity) - 0.5) * 2.0, -1.0, 1.0))
    role_uncertainty = float(np.clip(base.role_uncertainty * (1.0 - 0.20 * continuity_signal) + biology_uncertainty, 0.025, 0.35))
    active_probability = base.active_probability if inputs.active_probability is None else float(np.clip(inputs.active_probability, 0.0, 1.0))
    effectiveness = base.effectiveness_if_active if inputs.effectiveness_if_active is None else float(np.clip(inputs.effectiveness_if_active, 0.35, 1.10))

    compiled = replace(base, active_probability=active_probability, effectiveness_if_active=effectiveness, role_uncertainty=role_uncertainty, explosive_modifier=explosive_modifier, catchpoint_modifier=catchpoint_modifier, rushing_efficiency_modifier=rushing_modifier)
    trace = PlayerMechanismTrace(speed_signal=speed_signal, catchpoint_signal=catchpoint_signal, rushing_signal=rushing_signal, passing_signal=passing, ball_security_signal=ball_security, madden_attribute_count=len(inputs.madden_attributes.values), biology_uncertainty=biology_uncertainty, continuity_signal=continuity_signal, astrology_shadow_signal=inputs.astrology_shadow_signal)
...
def _weather_effect(inputs: TeamMechanismInputs) -> float:
    if inputs.dome:
        return 0.0
    wind_penalty = 0.0 if inputs.wind_mph is None else -0.0030 * max(float(inputs.wind_mph) - 10.0, 0.0)
    precip = 0.0 if inputs.precipitation_probability is None else float(np.clip(inputs.precipitation_probability, 0.0, 1.0))
...
def compile_team_mechanisms(team: TeamState, pool: TeamPlayerPool, inputs: TeamMechanismInputs) -> tuple[TeamState, TeamPlayerPool, TeamMechanismTrace]:
    ...
    personnel_signal = _weighted_signal(((madden_signal, 0.30), (line_signal, 0.40), (front_signal, 0.30)))
    ...
    compiled_team = replace(team, continuity=continuity, coaching_entropy=coaching_entropy, weather_effect=weather_effect, physical_madden_effect=personnel_effect)
    pass_weather_shift = float(np.clip(weather_effect * 0.35, -0.025, 0.0))
    compiled_pool = replace(pool, neutral_pass_rate=float(np.clip(pool.neutral_pass_rate + pass_weather_shift, 0.34, 0.72)), targetable_dropback_rate=float(np.clip(pool.targetable_dropback_rate + weather_effect * 0.20, 0.84, 0.98)), play_volume_uncertainty=float(np.clip(pool.play_volume_uncertainty + 0.03 * coaching_entropy - 0.015 * continuity_signal, 0.025, 0.18)))
    trace = TeamMechanismTrace(personnel_signal=personnel_signal, weather_effect=weather_effect, continuity_signal=continuity_signal, coaching_entropy=coaching_entropy)
