...
def _availability_weight(player: UnitPlayerInputs, snap_share: float) -> float:
    return float(np.clip(snap_share, 0.0, 1.0) * np.clip(player.active_probability, 0.0, 1.0) * np.clip(player.effectiveness_if_active, 0.35, 1.10))
...
def _skill_signal(player: UnitPlayerInputs, group: str) -> float | None:
    """Position-jurisdiction Madden talent proxy used only in the football scoring state."""
    if group == "QB":
        if all(v is None for v in (player.madden_throw_accuracy, player.madden_throw_power, player.madden_throw_under_pressure, player.madden_awareness)):
            return None
        return _mean_rating_signal(player.madden_throw_accuracy, player.madden_throw_power, player.madden_throw_under_pressure, player.madden_awareness)
    if group in {"WR", "TE"}:
        if all(v is None for v in (player.madden_route_running, player.madden_catching, player.madden_release, player.madden_speed, player.madden_acceleration)):
            return None
        return _mean_rating_signal(player.madden_route_running, player.madden_catching, player.madden_release, player.madden_speed, player.madden_acceleration)
    if group == "RB":
        if all(v is None for v in (player.madden_speed, player.madden_acceleration, player.madden_carrying, player.madden_break_tackle, player.madden_ball_carrier_vision)):
            return None
        return _mean_rating_signal(player.madden_speed, player.madden_acceleration, player.madden_carrying, player.madden_break_tackle, player.madden_ball_carrier_vision)
    return None
...
def compile_team_unit_effects(players: tuple[UnitPlayerInputs, ...]) -> tuple[TeamUnitEffects, UnitTrace]:
    ...
    for player in players:
        ...
        if offense_weight > 0:
            offense_players += 1; offense_trace_weight += offense_weight
            if group in {"OL", "TE", "RB"}:
                jurisdiction = 1.0 if group == "OL" else 0.35
                pass_block_values.append((_blend(player.pass_block_signal, player.madden_pass_block), offense_weight * jurisdiction))
            if group in {"OL", "TE", "WR"}:
                jurisdiction = 1.0 if group == "OL" else (0.40 if group == "TE" else 0.15)
                run_block_values.append((_blend(player.run_block_signal, player.madden_run_block), offense_weight * jurisdiction))
            skill = _skill_signal(player, group)
            if skill is not None:
                # QB receives greater jurisdiction over scoring quality; skill-position depth is
                # naturally snap-weighted. This remains a small bounded modifier, not a projection.
        ...
        if defense_weight > 0:
            defense_players += 1; defense_trace_weight += defense_weight
            if group in {"DL", "LB"}:
                pass_rush_values.append((_blend(player.pass_rush_signal, player.madden_pass_rush), defense_weight * (1.0 if group == "DL" else 0.55)))
            if group in {"DB", "LB"}:
                proxy = player.madden_coverage
                if proxy is None and player.madden_speed is not None: proxy = 0.70 * 78.0 + 0.30 * player.madden_speed
                coverage_values.append((_blend(player.coverage_signal, proxy), defense_weight * (1.0 if group == "DB" else 0.55)))
            if group in {"DL", "LB", "DB"}:
                run_defense_values.append((_blend(player.run_defense_signal, player.madden_tackle), defense_weight * (1.0 if group in {"DL", "LB"} else 0.45)))
            uncertainty_values.append((player.snap_share_uncertainty, defense_weight))
        if special_weight > 0:
            special_players += 1; special_trace_weight += special_weight
            kick = 0.60 * _rating_signal(player.madden_kick_accuracy) + 0.40 * _rating_signal(player.madden_kick_power)
            signal = float(np.clip(0.55 * _bounded_signal(player.special_teams_signal) + 0.30 * kick + 0.15 * _rating_signal(player.madden_return), -1, 1))
            special_values.append((signal, special_weight)); uncertainty_values.append((player.snap_share_uncertainty, special_weight))

    pass_block, _ = _weighted_average(pass_block_values); run_block, _ = _weighted_average(run_block_values)
    pass_rush, _ = _weighted_average(pass_rush_values); coverage, _ = _weighted_average(coverage_values)
    run_defense, _ = _weighted_average(run_defense_values); special, _ = _weighted_average(special_values)
    skill, _ = _weighted_average(skill_values); uncertainty, _ = _weighted_average(uncertainty_values)
    ...
    return effects, UnitTrace(offense_trace_weight, defense_trace_weight, special_trace_weight, offense_players, defense_players, special_players)
...
def apply_team_unit_effects(team: TeamState, effects: TeamUnitEffects) -> TeamState:
    return replace(
        ...
        physical_madden_effect=float(np.clip(team.physical_madden_effect + effects.skill_talent_effect, -0.08, 0.08)),
