...
def compile_player_mechanisms(
    ...
    """Compile rich player evidence into small, mechanism-specific modifiers.

    The compiler intentionally does not alter fantasy points. Age alone cannot reduce
    expected production. Astrology is recorded in the trace but has zero production
    authority.
    """
    ref = _position_reference(base.position)

    forty_signal = _signal(inputs.forty_time, ref["forty"], 0.16, reverse=True)
    madden_speed_signal = _signal(inputs.madden_speed, 85.0, 8.0)
    acceleration_signal = _signal(inputs.madden_acceleration, 85.0, 8.0)
    route_signal = _signal(inputs.madden_route_running, 82.0, 10.0)
    catching_signal = _signal(inputs.madden_catching, 82.0, 10.0)
    ...
    speed_signal = _weighted_signal(
        ((forty_signal, 0.55), (madden_speed_signal, 0.30), (acceleration_signal, 0.15))
    )
    catchpoint_signal = _weighted_signal(
        ((height_signal, 0.25), (wingspan_signal, 0.25), (catching_signal, 0.35), (route_signal, 0.15))
    )
    rushing_signal = _weighted_signal(
        ((forty_signal, 0.30), (madden_speed_signal, 0.20), (acceleration_signal, 0.20), (weight_signal, 0.30))
    )

    explosive_modifier = float(np.clip(base.explosive_modifier * (1.0 + 0.05 * speed_signal), 0.95, 1.05))
    catchpoint_modifier = float(np.clip(base.catchpoint_modifier * (1.0 + 0.05 * catchpoint_signal), 0.95, 1.05))
    rushing_modifier = float(
        np.clip(base.rushing_efficiency_modifier * (1.0 + 0.05 * rushing_signal), 0.95, 1.05)
    )

    # Biology affects uncertainty only unless direct health/effectiveness evidence exists.
    # An older player is not penalized merely for being older; workload must also be known
    # and elevated before this uncertainty term activates.
    ...
    role_uncertainty = float(
        np.clip(base.role_uncertainty * (1.0 - 0.20 * continuity_signal) + biology_uncertainty, 0.025, 0.35)
    )
    active_probability = (
        base.active_probability if inputs.active_probability is None else float(np.clip(inputs.active_probability, 0.0, 1.0))
    )
    effectiveness = (
...
def _weather_effect(inputs: TeamMechanismInputs) -> float:
    ...
    precip = 0.0 if inputs.precipitation_probability is None else float(
        np.clip(inputs.precipitation_probability, 0.0, 1.0)
    )
...
def compile_team_mechanisms(
    ...
    # Weather and continuity influence volume/availability of pass opportunities, not
    # fantasy scoring directly. Dome conditions nullify weather penalties upstream.
    pass_weather_shift = float(np.clip(weather_effect * 0.35, -0.025, 0.0))
    compiled_pool = replace(
        pool,
        neutral_pass_rate=float(np.clip(pool.neutral_pass_rate + pass_weather_shift, 0.34, 0.72)),
        ...
        play_volume_uncertainty=float(
            np.clip(pool.play_volume_uncertainty + 0.03 * coaching_entropy - 0.015 * continuity_signal, 0.025, 0.18)
        ),
    )
