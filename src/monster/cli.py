...
def validate_config(feature_registry: Path = Path("config/feature_registry.yaml")):
    ...
    typer.echo(
        f"features={len(registry.specs)} blind_allowed={len(registry.blind_allowed())} "
        f"forbidden={len(registry.forbidden_upstream())}"
    )
...
def free_budget():
    typer.echo(
        "Supabase target: <500MB Postgres, <1GB Storage; keep only canonical + rolling world artifacts"
    )
    typer.echo(
        "GitHub target: <2000 Actions min/month; heavy simulator workflow remains manual/on-demand"
    )
