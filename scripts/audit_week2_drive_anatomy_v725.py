        ...
        rows.append(
            {
                ...
                "actual_week2": a,
                "historical_2025": h,
                "v725_minus_actual": s - a,
                "v725_vs_actual_relative": _relative_delta(s, a),
                "v725_minus_historical": s - h,
                "v725_vs_historical_relative": _relative_delta(s, h),
...
def _team_comparison(
    ...
    for key in sorted(actual_groups):
        ...
        sim = _summary(sim_groups[key], team_games=worlds, scope=f"v725:{game}:{team}")
        output.append(
            {
                "game": game,
                "team": team,
                "actual_drives": actual["drives"],
                "v725_drives_mean": sim["drives_per_team_game"],
                "actual_points_per_drive": actual["points_per_drive"],
                "v725_points_per_drive": sim["points_per_drive"],
                ...
                "v725_offensive_points_mean": sim["offensive_points_per_team_game"],
                ...
                "v725_td_rate": sim["touchdown_rate"],
                "actual_punt_rate": actual["punt_rate"],
                "v725_punt_rate": sim["punt_rate"],
                "actual_turnover_rate": actual["turnover_rate"],
                "v725_turnover_rate": sim["turnover_rate"],
                "actual_yards_per_drive": actual["net_yards_per_drive"],
                "v725_yards_per_drive": sim["net_yards_per_drive"],
                "actual_rz_snap_rate": actual["red_zone_snap_rate"],
                "v725_rz_snap_rate": sim["red_zone_snap_rate"],
                "actual_rz_td_conversion": actual["td_per_red_zone_snap"],
                "v725_rz_td_conversion": sim["td_per_red_zone_snap"],
            }
        )
    return output
...
def main() -> None:
    ...
    sim_summary = _summary(
        sim_rows, team_games=games * 2 * worlds, scope="v725_week2_simulation"
    )
    ...
    structural.sort(
        key=lambda row: abs(float(row["v725_vs_historical_relative"] or 0.0)),
        reverse=True,
    )
    week2_specific = [
    ...
    diagnosis = {
        ...
        "v725_vs_2025_points_per_drive_relative": _relative_delta(
            float(sim_summary["points_per_drive"]),
    ...
    segment_rows = _segment_rows(
        sim_rows, team_games=games * 2 * worlds, source="v725"
    ) + _segment_rows(
    ...
    manifest = {
        "artifact": "MONSTER V7.2.5 Week 2 Drive Anatomy Reality Audit",
        "season": args.season,
        "week": args.week,
        "games": games,
