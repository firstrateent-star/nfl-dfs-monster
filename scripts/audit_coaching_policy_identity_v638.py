...
def main() -> None:
    ...
    renames = (
        (
            ...
            "v638_coaching_policy_identity_matrix.csv",
        ),
        (
            "v635_coaching_policy_identity_summary.csv",
            "v638_coaching_policy_identity_summary.csv",
        ),
    )
    ...
    new_report = out / "v638_coaching_policy_identity.json"
    payload = json.loads(old_report.read_text(encoding="utf-8"))
    payload["experiment"] = "v6.3.8-coaching-policy-identity"
    payload["audit_logic"] = "exact v6.3.5 coaching audit, v6.3.8 runtime injected"
