...
class GameWorlds:
    ...
    # Event-first engines may provide the exact finite scrimmage-play supply and typed scoring
    # anatomy that generated the scoreboard. Legacy v1 leaves these unset and preserves its
    # certified downstream allocation behavior.
    away_plays: np.ndarray | None = None
    home_plays: np.ndarray | None = None
    away_passing_touchdowns: np.ndarray | None = None
    home_passing_touchdowns: np.ndarray | None = None
    away_rushing_touchdowns: np.ndarray | None = None
    home_rushing_touchdowns: np.ndarray | None = None
...
def _simulate_team_drives(
    ...
    pass_matchup = team.pass_protection_effect - opponent.pass_rush_effect - 0.70 * opponent.coverage_effect
    ...
    if team.offensive_line_uncertainty <= 0.0:
        ...
        line_sigma = float(np.clip(team.offensive_line_uncertainty + 0.035 * (1.0 - team.offensive_line_continuity), 0.0, 0.20))
        line_state = rng.normal(0.0, line_sigma, worlds).astype(np.float32)
    pass_disruption = np.clip(1.0 + 1.10 * (opponent.pass_rush_effect + 0.60 * opponent.coverage_effect - team.pass_protection_effect) - 0.80 * line_state, 0.78, 1.22).astype(np.float32)
    run_efficiency = np.clip(1.0 + 0.90 * run_matchup + 0.60 * line_state, 0.84, 1.16).astype(np.float32)
    quality_base = (1.00 + 0.55 * team.offensive_epa_per_play + 0.45 * opponent.defensive_epa_allowed_per_play + 0.22 * (team.offensive_explosive_rate - 0.10) - 0.18 * (opponent.defensive_explosive_rate_allowed - 0.10) + 0.15 * (team.red_zone_td_rate - 0.55) + unit_matchup + 0.25 * team.special_teams_effect + team.injury_effect + team.weather_effect + team.physical_madden_effect)
    ...
    to_p = _clip_probability((to_base * turnover_multiplier) / np.sqrt(np.maximum(quality * state, 0.35)), 0.035, 0.24)
    fg_p = _clip_probability(fg_base * (1.0 + team.special_teams_effect) * (0.92 + 0.08 * quality) * np.sqrt(state), 0.05, 0.28)
    ...
    return points, touchdowns, field_goals, turnovers, pass_disruption, run_efficiency
...
def simulate_game(game: GameState, worlds: int, seed: int) -> GameWorlds:
    ...
    return GameWorlds(
        away_points=away[0], home_points=home[0], away_drives=away_drives, home_drives=home_drives,
        away_touchdowns=away[1], home_touchdowns=home[1], away_field_goals=away[2], home_field_goals=home[2],
        away_turnovers=away[3], home_turnovers=home[3], away_pass_disruption=away[4], home_pass_disruption=home[4],
        away_run_efficiency=away[5], home_run_efficiency=home[5],
    )
