...
def runtime_fingerprint_v637() -> RuntimeFingerprintV637:
    payload = _fingerprint_payload()
    digest = hashlib.sha256(
        json.dumps(payload, sort_keys=True, separators=(",", ":")).encode("utf-8")
    ).hexdigest()
    return RuntimeFingerprintV637(**payload, runtime_hash=digest)
...
def _runtime_integrity_errors(*, require_role_guard: bool) -> list[str]:
    ...
    if require_role_guard:
        if reality_v62.sample_target_share_plan is not sample_target_share_plan_v635:
            errors.append("v6.3.5 target role sampler changed inside v6.3.7")
        if reality_v62.sample_event_rush_share_plan is not sample_rush_share_plan_v637:
            errors.append("v6.3.7 rush role sampler is not active after personnel compile")
    return errors


def compose_v637_runtime() -> None:
    """Install v6.3.5 production reality plus the isolated v6.3.7 rush fix."""

    v634.install_current_role_guard = install_current_role_guard_v637
    ...
    if errors:
        raise RuntimeError(
            "v6.3.7 static runtime integrity failure: " + "; ".join(errors)
        )


def assert_v637_runtime() -> RuntimeFingerprintV637:
    errors = _runtime_integrity_errors(require_role_guard=True)
    if errors:
        raise RuntimeError("v6.3.7 runtime integrity failure: " + "; ".join(errors))
    return runtime_fingerprint_v637()


def assert_team_runtime_v637(teams: dict[str, object]) -> None:
    missing_flow = [
        team_id
...
def build_week1_runtime_inputs_v637(
    *,
    ...
    assert_team_runtime_v637(teams)
    fingerprint = assert_v637_runtime()
...
def write_runtime_fingerprint_v637(out: Path) -> RuntimeFingerprintV637:
    fingerprint = assert_v637_runtime()
    out.mkdir(parents=True, exist_ok=True)
    (out / "runtime_fingerprint_v637.json").write_text(
