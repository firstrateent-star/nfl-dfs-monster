...
def simulate_monster_game(
    ...
    *,
    ...
    game_worlds = simulate_event_game(
        snapshot.game,
        snapshot.away_pool,
        snapshot.home_pool,
        worlds=worlds,
    ...
    allocation = allocate_event_game_players(
        game_worlds,
        snapshot.away_pool,
        snapshot.home_pool,
        seed=seed + 1_000_003,
    )
    allocation = refine_game_receiving_roles(
        allocation,
        snapshot.away_pool,
        snapshot.home_pool,
        seed=seed + 2_000_021,
    )
    allocation = refine_game_rushing_roles(
        allocation,
        snapshot.away_pool,
        snapshot.home_pool,
        seed=seed + 3_000_033,
    )
    # Refinement is allowed to redistribute finite opportunity, never to redefine the game.
