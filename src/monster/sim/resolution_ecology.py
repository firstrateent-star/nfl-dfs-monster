...
def depth_throw_probabilities(
    ...
    *,
    ...
    if matchup_completion_probability is not None:
        relative = float(np.clip(matchup_completion_probability / 0.64, 0.72, 1.30))
        completion *= relative
    if matchup_interception_probability is not None:
        relative_int = float(np.clip(matchup_interception_probability / 0.022, 0.55, 1.70))
    ...
    return DepthThrowProbabilities(completion=completion, interception=interception)
...
def resolve_run_ecology(
    ...
    *,
    ...
    blocking/front/runner/tackling interaction shifts branch odds within bounded limits.
    This creates both meaningful TFLs and genuine breakaways instead of forcing all runs
    through one narrow Gaussian contact distribution.
    """
    ...
    win_factor = float(
        np.clip(
            ...
            * max(explosiveness, 0.65)
            * max(runner_power, 0.65)
            / max(tackling**0.30, 0.80),
            0.55,
            1.65,
        )
    )

    negative_p = float(np.clip(profile.negative_rate * stuff_factor, 0.025, 0.24))
    zero_p = float(np.clip(profile.zero_rate * stuff_factor**0.35, 0.015, 0.16))
    explosive_20_p = float(np.clip(profile.explosive_20_rate * win_factor, 0.001, 0.12))
    explosive_15_p = float(
        np.clip(max(profile.explosive_15_rate - profile.explosive_20_rate, 0.0) * win_factor, 0.0, 0.12)
    )
    explosive_10_p = float(
        np.clip(max(profile.explosive_10_rate - profile.explosive_15_rate, 0.0) * win_factor, 0.0, 0.18)
    )
    ...
    draw = rng.random()
    if draw < negative_p:
        conditional_loss_5 = profile.loss_5_plus_rate / max(profile.negative_rate, 1e-9)
        conditional_loss_2 = profile.loss_2_plus_rate / max(profile.negative_rate, 1e-9)
        severity_draw = rng.random()
        if severity_draw < conditional_loss_5:
            yards = -float(np.clip(rng.lognormal(1.70, 0.35), 5.0, 14.0))
    ...
    if draw < explosive_20_p:
        ...
        yards = float(np.clip(rng.lognormal(np.log(center), 0.38), 20.0, 75.0))
    ...
    routine_center = float(np.clip(profile.yards_p50, 2.0, 5.5))
