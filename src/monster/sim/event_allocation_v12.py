...
def allocate_event_game_players(
    ...
    *,
    ...
    only resolves who participated in the already-simulated football. Final TD ownership is
    coupled after role refinement so no later stage can detach a TD from its source event.
    """
    required = (
    ...
    away_disruption = game_worlds.away_pass_disruption if game_worlds.away_pass_disruption is not None else np.ones(worlds, dtype=np.float32)
    home_disruption = game_worlds.home_pass_disruption if game_worlds.home_pass_disruption is not None else np.ones(worlds, dtype=np.float32)
    away_run_eff = game_worlds.away_run_efficiency if game_worlds.away_run_efficiency is not None else np.ones(worlds, dtype=np.float32)
    home_run_eff = game_worlds.home_run_efficiency if game_worlds.home_run_efficiency is not None else np.ones(worlds, dtype=np.float32)

    away = _allocate_team(rng, away_pool, game_worlds.away_points, game_worlds.home_points, game_worlds.away_plays, game_worlds.away_touchdowns, away_disruption, away_run_eff)
    home = _allocate_team(rng, home_pool, game_worlds.home_points, game_worlds.away_points, game_worlds.home_plays, game_worlds.home_touchdowns, home_disruption, home_run_eff)
