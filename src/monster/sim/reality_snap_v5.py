    ...
    *,
    ...
    for player in players:
        ...
        u = max(
            (
                int.from_bytes(
                    blake2b(f"{key}:{_pid(player)}".encode(), digest_size=8).digest(),
                    "big",
                )
                + 0.5
            )
...
def _package_probabilities(
    *, down: int, distance: float, neutral_pass_rate: float
...
def _select_package(
    key: str, *, down: int, distance: float, neutral_pass_rate: float
) -> str:
...
def _offense_skill_players(
    ...
    for positions, count, label in groups:
        pool = tuple(
            player
            for player in players
            if _pos(player) in positions and _pid(player) not in selected_ids
        )
    ...
    if len(selected) < 5:
        ...
        picks = _weighted_without_replacement(
...
def _offense_alignment(
    ...
    rb_roles = ("BACK", "HBACK")
    rows.extend(
        f"{_pid(player)}:{wr_roles[min(idx, len(wr_roles) - 1)]}"
        for idx, player in enumerate(wr)
    )
    rows.extend(
        f"{_pid(player)}:{te_roles[min(idx, len(te_roles) - 1)]}"
        for idx, player in enumerate(te)
    )
    rows.extend(
        f"{_pid(player)}:{rb_roles[min(idx, len(rb_roles) - 1)]}"
        for idx, player in enumerate(rb)
    )
    return tuple(rows)


def _defense_counts(
    package: str, *, down: int, distance: float
...
def _defense_participants(
    ...
    *,
    ...
    defense_package, dl_need, lb_need, db_need = _defense_counts(
        package, down=down, distance=distance
    )
    ...
    dl = tuple(
        ...
        if _pos(player) in {"EDGE", "DE", "DT", "NT", "DL"}
    )
    lb = tuple(
        player
        for player in values
        if _pos(player) in {"LB", "ILB", "MLB", "OLB"}
    )
    db = tuple(
        player
        for player in values
        if _pos(player) in {"CB", "DB", "S", "FS", "SS"}
    )
    selected: list[object] = []
    selected_ids: set[str] = set()
    for pool, need, label in (
        (dl, dl_need, "dl"),
        (lb, lb_need, "lb"),
        (db, db_need, "db"),
    ):
        ...
        picks = _weighted_without_replacement(
            pool, need, key=f"{key}:def:{label}", exposure=True
        )
        selected.extend(picks)
        selected_ids.update(_pid(player) for player in picks)
    if len(selected) < 11:
        ...
        selected.extend(
            _weighted_without_replacement(
                pool, 11 - len(selected), key=f"{key}:def:fallback", exposure=True
            )
        )
    ...
    alignment.extend(
        f"{_pid(player)}:{dl_roles[min(idx, len(dl_roles) - 1)]}"
        for idx, player in enumerate(dl_rows)
    )
    alignment.extend(
        f"{_pid(player)}:{lb_roles[min(idx, len(lb_roles) - 1)]}"
        for idx, player in enumerate(lb_rows)
    )
    alignment.extend(
        f"{_pid(player)}:{db_roles[min(idx, len(db_roles) - 1)]}"
        for idx, player in enumerate(db_rows)
    )
    return defense_package, tuple(selected), tuple(alignment)


def _intent_for_snap(
    key: str, *, state: object, offense: object
) -> DefensiveIntent:
    qb_threat = float(
        np.clip(
...
def _team_profile_for_offense(
    offense: object, skill: tuple[object, ...]
):
    """Recover the registered OL through any known offensive participant.

    The OL registry is keyed by roster player IDs. Synthetic tests may register only OL/RB/TE
    unit rows, while production usually registers the broader unit roster. Looking across the
    offense makes the snap-world contract team-scoped without incorrectly requiring the QB to
    be one of the rows used to compile blocking evidence.
    """
    candidates = (
        getattr(offense, "quarterback", None),
        *skill,
        *tuple(getattr(offense, "receivers", ())),
        *tuple(getattr(offense, "rushers", ())),
    )
...
def prepare_snap_world(
    *,
    ...
    receivers = tuple(
        player
        for player in getattr(offense, "receivers", ())
        if _pid(player) in active_ids
    )
    rushers = tuple(
        player
        for player in getattr(offense, "rushers", ())
        if _pid(player) in active_ids or _pos(player) == "QB"
    )
    if not receivers:
    ...
    profile = _team_profile_for_offense(offense, skill)
    blockers = () if profile is None else profile.offensive_line
    quarterback_id = str(getattr(offense.quarterback, "player_id", ""))
    offense_participants = (
        (quarterback_id,)
        + tuple(_pid(blocker) for blocker in blockers)
        + tuple(_pid(player) for player in skill)
    )
    offense_alignment = _offense_alignment(skill, blockers, quarterback_id)
    ...
    if defense is not None:
        ...
        active_front = tuple(
            player for player in defense.front if _pid(player) in selected_ids
        )
        active_coverage = tuple(
            player for player in defense.coverage if _pid(player) in selected_ids
        )
        active_defense = replace(
            defense, front=active_front, coverage=active_coverage
        )
...
def planned_rushers(
    responsibility_key: str, fallback: tuple[object, ...]
) -> tuple[object, ...]:
    world = snap_world(responsibility_key)
    if world is None or not world.rush_participant_ids:
        return fallback
    selected = tuple(
        player for player in fallback if _pid(player) in world.rush_participant_ids
    )
    return selected or fallback


def record_pass_resolution(
    responsibility_key: str, target_id: str, **values: Any
) -> None:
...
def record_trench_duels(
    responsibility_key: str, duels: tuple[object, ...]
) -> None:
    evidence = _EVIDENCE_BY_KEY.setdefault(responsibility_key, SnapEvidenceV5())
...
def coverage_intent_adjustments(
    responsibility_key: str,
...
def run_fit_multiplier(responsibility_key: str) -> float:
    ...
    return float(
        np.clip(1.0 + 0.09 * intent.authority * centered, 0.94, 1.06)
    )
